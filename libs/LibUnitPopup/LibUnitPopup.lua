L_UNITPOPUP_TITLE_HEIGHT = 26;
L_UNITPOPUP_BUTTON_HEIGHT = 15;
L_UNITPOPUP_BORDER_HEIGHT = 8;
L_UNITPOPUP_BORDER_WIDTH = 19;

L_UNITPOPUP_NUMBUTTONS = 9;
L_UNITPOPUP_TIMEOUT = 5;

L_UNITPOPUP_SPACER_SPACING = 6;
...
for which, info in pairs(L_UnitPopupMenus) do
	for index, value in pairs(info) do
		if taints[value] then
			tremove(info, index)
		end
	end
end
...
local function L_UnitPopup_CheckAddSubsection(dropdownMenu, info, menuLevel, currentButton, previousButton, previousIndex, previousValue)
	if previousButton and previousButton.isSubsection then
		if not currentButton.isSubsection then
			...
			if previousButton.isSubsectionTitle and info then
				L_UnitPopup_AddDropDownButton(info, dropdownMenu, previousButton, previousValue, menuLevel);
			end
		else
			L_UnitPopupShown[menuLevel][previousIndex] = 0;
		end
	end
end
...
function L_UnitPopup_HasVisibleMenu()
	return g_mostRecentPopupMenu == L_UIDROPDOWNMENU_OPEN_MENU;
end

local function L_UnitPopup_GetBNetIDAccount(dropdownMenu)
...
local function L_UnitPopup_GetGUID(menu)
	if menu.guid then
		return menu.guid;
...
local function L_UnitPopup_GetBNetAccountInfo(menu)
	local bnetIDAccount = L_UnitPopup_GetBNetIDAccount(menu)
	if bnetIDAccount then
		return C_BattleNet.GetAccountInfoByID(bnetIDAccount);
	else
		local guid = L_UnitPopup_GetGUID(menu);
		if guid then
...
local function L_UnitPopup_GetIsMobile(menu)
...
function L_UnitPopup_ShowMenu (dropdownMenu, which, unit, name, userData)
	...
	dropdownMenu.accountInfo = L_UnitPopup_GetBNetAccountInfo(dropdownMenu);
	dropdownMenu.isMobile = L_UnitPopup_GetIsMobile(dropdownMenu);

	-- Determine which buttons should be shown or hidden
	L_UnitPopup_HideButtons();

	-- If only one menu item (the cancel button) then don't show the menu
	local count = 0;
	for index, value in ipairs(L_UnitPopupMenus[L_UIDROPDOWNMENU_MENU_VALUE] or L_UnitPopupMenus[which]) do
		if( L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] == 1 and not L_UnitPopupButtons[value].isCloseCommand ) then
			count = count + 1;
		end
	end
	if ( count < 1 ) then
	...
	L_UnitPopupButtons["GARRISON_VISIT"].text = (C_Garrison.IsUsingPartyGarrison() and GARRISON_RETURN) or GARRISON_VISIT_LEADER;
	...
	if ( not inInstance ) then
		L_UnitPopupButtons["DUNGEON_DIFFICULTY"].nested = 1;
		if( inPublicParty ) then
			L_UnitPopupButtons["RAID_DIFFICULTY"].nested = nil;
		else
			L_UnitPopupButtons["RAID_DIFFICULTY"].nested = 1;
		end
	else
		if (instanceType == "raid") then
			L_UnitPopupButtons["RAID_DIFFICULTY"].nested = 1;
			L_UnitPopupButtons["DUNGEON_DIFFICULTY"].nested = nil;
		else
			L_UnitPopupButtons["DUNGEON_DIFFICULTY"].nested = 1;
			L_UnitPopupButtons["RAID_DIFFICULTY"].nested = nil;
		end
	...
	local specPopupButton = L_UnitPopupButtons["LOOT_SPECIALIZATION_DEFAULT"];
	...
	for index = 1, 4 do
		specPopupButton = L_UnitPopupButtons["LOOT_SPECIALIZATION_SPEC"..index];
	...
	do
		...
		if ( remaining > 0 ) then
			L_UnitPopupButtons["RAF_SUMMON"].text = format(RAF_SUMMON_WITH_COOLDOWN, SecondsToTime(remaining, true));
		else
			L_UnitPopupButtons["RAF_SUMMON"].text = RAF_SUMMON;
		end
	end
	...
	if ( L_UIDROPDOWNMENU_MENU_LEVEL == 2 ) then
		...
		for index, value in ipairs(L_UnitPopupMenus[L_UIDROPDOWNMENU_MENU_VALUE]) do
			if( L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] == 1 ) then
				local cntButton = L_UnitPopupButtons[value];

				L_UnitPopup_CheckAddSubsection(dropdownMenu, info, L_UIDROPDOWNMENU_MENU_LEVEL, cntButton, previousButton, previousIndex, previousValue);
				...
				info.text = L_UnitPopupButtons[value].text;
				info.owner = L_UIDROPDOWNMENU_MENU_VALUE;
				-- Set the text color
				color = L_UnitPopupButtons[value].color;
				if ( color ) then
				...
				-- Icons
				info.icon = L_UnitPopupButtons[value].icon;
				info.tCoordLeft = L_UnitPopupButtons[value].tCoordLeft;
				info.tCoordRight = L_UnitPopupButtons[value].tCoordRight;
				info.tCoordTop = L_UnitPopupButtons[value].tCoordTop;
				info.tCoordBottom = L_UnitPopupButtons[value].tCoordBottom;
				-- Checked conditions
				info.checked = nil;
				...
				elseif ( strsub(value, 1, 18) == "DUNGEON_DIFFICULTY" and (strlen(value) > 18)) then
					...
					if ( dungeonDifficultyID == L_UnitPopupButtons[value].difficultyID ) then
						info.checked = true;
					end
				...
				elseif (strsub(value, 1, 15) == "RAID_DIFFICULTY" and (strlen(value) > 15) ) then
					if ( isDynamicInstance ) then
						...
						if ( IsLegacyDifficulty(instanceDifficultyID) ) then
							...
							if ((instanceDifficultyID == DifficultyUtil.ID.Raid10Normal or instanceDifficultyID == DifficultyUtil.ID.Raid25Normal) and L_UnitPopupButtons[value].difficultyID == DifficultyUtil.ID.PrimaryRaidNormal) then
								info.checked = true;
							elseif ((instanceDifficultyID == DifficultyUtil.ID.Raid10Heroic or instanceDifficultyID == DifficultyUtil.ID.Raid25Heroic) and L_UnitPopupButtons[value].difficultyID == DifficultyUtil.ID.PrimaryRaidHeroic) then
								info.checked = true;
							end
						elseif ( instanceDifficultyID == L_UnitPopupButtons[value].difficultyID ) then
							info.checked = true;
						end
					else
						local raidDifficultyID = GetRaidDifficultyID();
						if ( raidDifficultyID == L_UnitPopupButtons[value].difficultyID ) then
							info.checked = true;
						end
					end
					...
					if ( toggleDifficultyID and L_CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID) ) then
						info.disabled = nil;
					end
				elseif (strsub(value, 1, 22) == "LEGACY_RAID_DIFFICULTY" and (strlen(value) > 15) ) then
					if ( isDynamicInstance ) then
						if ( NormalizeLegacyDifficultyID(instanceDifficultyID) == L_UnitPopupButtons[value].difficultyID ) then
							info.checked = true;
						end
					else
						local raidDifficultyID = GetLegacyRaidDifficultyID();
						if ( NormalizeLegacyDifficultyID(raidDifficultyID) == L_UnitPopupButtons[value].difficultyID ) then
							info.checked = true;
						end
					end
					...
					if ( toggleDifficultyID and not GetRaidDifficultyID() == DifficultyUtil.ID.PrimaryRaidMythic and L_CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID) ) then
						info.disabled = nil;
					end
				...
				elseif ( strsub(value, 1, 20) == "LOOT_SPECIALIZATION_" ) then
					if ( GetLootSpecialization() == L_UnitPopupButtons[value].specializationID ) then
						info.checked = true;
					end
				...
				info.value = value;
				info.func = L_UnitPopup_OnClick;
				if ( not L_UnitPopupButtons[value].checkable ) then
					info.notCheckable = true;
				else
					info.notCheckable = nil;
				end
				if ( L_UnitPopupButtons[value].isNotRadio ) then
					info.isNotRadio = true;
				else
					info.isNotRadio = nil;
				end
				-- Setup newbie tooltips
				info.tooltipTitle = L_UnitPopupButtons[value].text;
				info.tooltipText = _G["NEWBIE_TOOLTIP_UNIT_"..value];

				if not cntButton.isSubsection then
					L_UnitPopup_AddDropDownButton(info, dropdownMenu, cntButton, value, L_UIDROPDOWNMENU_MENU_LEVEL);
				end
	...
	L_UnitPopup_AddDropDownTitle(unit, name, userData);
	...
	local tooltipText;
	local previousButton, previousIndex, previousValue;
	for index, value in ipairs(L_UnitPopupMenus[which]) do
		if( L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] == 1 ) then
			local cntButton = L_UnitPopupButtons[value];
			...
			if not cntButton.isSubsection then
				L_UnitPopup_AddDropDownButton(info, dropdownMenu, cntButton, value);
			end
...
function L_UnitPopup_AddDropDownTitle(unit, name, userData)
...
function L_UnitPopup_GetOverrideIsChecked(command, currentIsChecked, dropdownMenu)
...
function L_UnitPopup_UpdateButtonInfo(info)
...
local function L_UnitPopup_TryCreatePlayerLocation(menu, guid)
...
function L_UnitPopup_AddDropDownButton(info, dropdownMenu, cntButton, buttonIndex, level)
	if (not level) then
		level = 1;
	end
	...
	info.checked = L_UnitPopup_GetOverrideIsChecked(buttonIndex, info.checked, dropdownMenu);
	...
	if ( cntButton.isTitle ) then
		...
		-- NOTE: L_UnitPopup_AddDropDownButton is called for both level 1 and 2 buttons, level 2 buttons already
	...
	if info.customFrame then
		local guid = L_UnitPopup_GetGUID(dropdownMenu);
		local playerLocation = L_UnitPopup_TryCreatePlayerLocation(dropdownMenu, guid);
		local contextData = {
			guid = guid,
	...
	L_UnitPopup_UpdateButtonInfo(info);

	L_UIDropDownMenu_AddButton(info, level);
end

local function L_UnitPopup_IsValidPlayerLocation(playerLocation)
	return playerLocation and playerLocation:IsValid();
end

local function L_UnitPopup_IsSameServer(playerLocation, dropdownMenu)
...
local function L_UnitPopup_HasBattleTag()
...
local function L_UnitPopup_GetLFGCategoryForLFGSlot(lfgSlot)
	if lfgSlot then
		return GetLFGCategoryForID(lfgSlot);
	end
end

local function L_UnitPopup_IsPlayerOffline(menu)
...
local function L_UnitPopup_IsPlayerFavorite(menu)
	return menu.accountInfo and menu.accountInfo.isFavorite;
end

local function L_UnitPopup_IsPlayerMobile(menu)
...
local function L_UnitPopup_GetIsLocalPlayer(menu)
	if menu.isSelf then
		return true;
	end
...
local function L_UnitPopup_IsInGroupWithPlayer(dropdownMenu)
...
local function L_UnitPopup_IsBNetFriend(dropdownMenu)
	return dropdownMenu.accountInfo and dropdownMenu.accountInfo.isFriend;
end

local function L_UnitPopup_CanAddBNetFriend(dropdownMenu, isLocalPlayer, haveBattleTag, isPlayer)
	local hasClubInfo = dropdownMenu.clubInfo ~= nil and dropdownMenu.clubMemberInfo ~= nil;
	return not isLocalPlayer and haveBattleTag and (isPlayer or hasClubInfo or dropdownMenu.accountInfo) and not L_UnitPopup_IsBNetFriend(dropdownMenu);
end

function L_UnitPopup_HideButtons ()
	...
	local partyLFGCategory = L_UnitPopup_GetLFGCategoryForLFGSlot(partyLFGSlot);
	local guid = L_UnitPopup_GetGUID(dropdownMenu);
	local playerLocation = L_UnitPopup_TryCreatePlayerLocation(dropdownMenu, guid);
	local isSameServer = L_UnitPopup_IsSameServer(playerLocation, dropdownMenu);
	local haveBattleTag = L_UnitPopup_HasBattleTag();
	local isOffline = L_UnitPopup_IsPlayerOffline(dropdownMenu);
	local isBNFriend = L_UnitPopup_IsBNetFriend(dropdownMenu);
	local isBNFriendFavorite = L_UnitPopup_IsPlayerFavorite(dropdownMenu);
	local isValidPlayerLocation = L_UnitPopup_IsValidPlayerLocation(playerLocation);
	local isLocalPlayer = L_UnitPopup_GetIsLocalPlayer(dropdownMenu);

	for index, value in ipairs(L_UnitPopupMenus[L_UIDROPDOWNMENU_MENU_VALUE] or L_UnitPopupMenus[dropdownMenu.which]) do
		local shown = true;
		if ( value == "TRADE" ) then
		...
		elseif ( value == "GUILD_BATTLETAG_FRIEND" ) then
			if ( not L_UnitPopup_CanAddBNetFriend(dropdownMenu, isLocalPlayer, haveBattleTag, isPlayer) ) then
				shown = false;
			end
	   ...
	   elseif ( value == "MOVE_FOCUS_FRAME" ) then
		...
		elseif ( strsub(value, 1, 20) == "LOOT_SPECIALIZATION_" ) then
			if ( L_UnitPopupButtons[value].specializationID == -1 ) then
				shown = false;
			end
		...
		elseif value == "COMMUNITIES_BATTLETAG_FRIEND" then
			if not haveBattleTag
				or not L_UnitPopup_CanAddBNetFriend(dropdownMenu, isLocalPlayer, haveBattleTag, isPlayer)
		...
		L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] = shown and 1 or 0;
	end
end

local function L_UnitPopup_IsEnabled(dropdownFrame, unitPopupButton)
...
function L_UnitPopup_OnUpdate (elapsed)
	if ( not L_DropDownList1:IsShown() ) then
		return;
	end

	if ( not L_UnitPopup_HasVisibleMenu() ) then
		return;
	end
	...
	local isLeader = UnitIsGroupLeader("player");
	local isAssistant = UnitIsGroupAssistant("player");
	local guid = L_UnitPopup_GetGUID(currentDropDown);
	local playerLocation = L_UnitPopup_TryCreatePlayerLocation(currentDropDown, guid);
	local isSameServer = L_UnitPopup_IsSameServer(playerLocation, currentDropDown);
	local isLocalPlayer = L_UnitPopup_GetIsLocalPlayer(currentDropDown);
	local haveBattleTag = L_UnitPopup_HasBattleTag();
	local isPlayer = currentDropDown.unit and UnitIsPlayer(currentDropDown.unit);
	local isInGroupWithPlayer = L_UnitPopup_IsInGroupWithPlayer(currentDropDown);
	...
	for level, dropdownFrame in pairs(L_OPEN_DROPDOWNMENUS) do
		if ( dropdownFrame ) then
			count = 0;
			for index, value in ipairs(L_UnitPopupMenus[dropdownFrame.which]) do
				if ( L_UnitPopupShown[level][index] == 1 ) then
					count = count + 1;
					local enable = L_UnitPopup_IsEnabled(dropdownFrame, L_UnitPopupButtons[value]);
					...
					elseif ( ( strsub(value, 1, 15) == "RAID_DIFFICULTY" ) and ( strlen(value) > 15 ) ) then
						...
						if (toggleDifficultyID) then
							enable = L_CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID);
						end
					elseif ( ( strsub(value, 1, 22) == "LEGACY_RAID_DIFFICULTY" ) and ( strlen(value) > 22 ) ) then
						...
						if (toggleDifficultyID) then
							if (IsLegacyDifficulty(toggleDifficultyID)) then
								notClickable = L_CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID);
							end
							enable = false;
						end
					elseif ( value == "CONVERT_TO_PARTY" ) then
					...
					elseif ( value == "BATTLETAG_FRIEND" ) then
						if ( not L_UnitPopup_CanAddBNetFriend(currentDropDown, isLocalPlayer, haveBattleTag, isPlayer) or not BNFeaturesEnabledAndConnected()) then
							enable = false;
						end
					elseif ( value == "GUILD_BATTLETAG_FRIEND" ) then
					...
					if ( L_UnitPopupButtons[value].isSubsectionTitle ) then
						--If the button is a title then it has a separator above it that is not in L_UnitPopupButtons.
						--So 1 extra is added to each count because L_UnitPopupButtons does not count the separators and
						--the DropDown does.
...
function L_UnitPopup_OnClick (self)
	...
	local guid = L_UnitPopup_GetGUID(dropdownFrame);
	local playerLocation = L_UnitPopup_TryCreatePlayerLocation(dropdownFrame, guid);
	...
	elseif ( strsub(button, 1, 20) == "LOOT_SPECIALIZATION_" ) then
		SetLootSpecialization(L_UnitPopupButtons[button].specializationID);
	elseif ( strsub(button, 1, 18) == "DUNGEON_DIFFICULTY" and (strlen(button) > 18) ) then
		local dungeonDifficultyID = L_UnitPopupButtons[button].difficultyID;
		SetDungeonDifficultyID(dungeonDifficultyID);
	elseif ( strsub(button, 1, 15) == "RAID_DIFFICULTY" and (strlen(button) > 15)) then
		local raidDifficultyID = L_UnitPopupButtons[button].difficultyID;
		L_SetRaidDifficulties(true, raidDifficultyID);
	elseif ( strsub(button, 1, 22) == "LEGACY_RAID_DIFFICULTY" and (strlen(button) > 22)) then
		local raidDifficultyID = L_UnitPopupButtons[button].difficultyID;
		L_SetRaidDifficulties(false, raidDifficultyID);
	...
	elseif ( button == "ITEM_QUALITY2_DESC" or button == "ITEM_QUALITY3_DESC" or button == "ITEM_QUALITY4_DESC" ) then
		...
		SetLootThreshold(id);
		L_UIDropDownMenu_SetButtonText(self:GetParent().parentLevel, self:GetParent().parentID, L_UnitPopupButtons[button].text, ITEM_QUALITY_COLORS[id].hex);
...
local function GetMappedLegacyDifficultyID(difficultyID, size)
	for i, mappedDifficultyID in ipairs(L_UIDROPDOWNMENU_[difficultyID]) do
...
function L_SetRaidDifficulties(primaryRaid, difficultyID)
	if primaryRaid then
...
function L_CheckToggleDifficulty(toggleDifficultyID, difficultyID)
	if (IsLegacyDifficulty(toggleDifficultyID)) then
		if (not IsLegacyDifficulty(difficultyID)) then
			return tContains(L_UIDROPDOWNMENU_[difficultyID], toggleDifficultyID);
		else
