...
local function makeL_UnitPopupSubsectionTitle(titleText)
...
L_UnitPopupButtons = {
	...
	["LOOT_SUBSECTION_TITLE"] = makeL_UnitPopupSubsectionTitle(UNIT_FRAME_DROPDOWN_SUBSECTION_TITLE_LOOT),
	["INSTANCE_SUBSECTION_TITLE"] = makeL_UnitPopupSubsectionTitle(UNIT_FRAME_DROPDOWN_SUBSECTION_TITLE_INSTANCE),
	["OTHER_SUBSECTION_TITLE"] = makeL_UnitPopupSubsectionTitle(UNIT_FRAME_DROPDOWN_SUBSECTION_TITLE_OTHER),
	["INTERACT_SUBSECTION_TITLE"] = makeL_UnitPopupSubsectionTitle(UNIT_FRAME_DROPDOWN_SUBSECTION_TITLE_INTERACT),
	["LEGACY_RAID_SUBSECTION_TITLE"] = makeL_UnitPopupSubsectionTitle(UNIT_FRAME_DROPDOWN_SUBSECTION_TITLE_LEGACY_RAID),
...
function L_UnitPopup_ShowMenu (dropdownMenu, which, unit, name, userData)
	...
	for index, value in ipairs(L_UnitPopupMenus[L_UIDROPDOWNMENU_MENU_VALUE] or L_UnitPopupMenus[which]) do
		...
		if( L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] == 1 and value ~= "CANCEL" ) then
			count = count + 1;
		end
	end
	if ( count < 1 ) then
	...
	L_UnitPopupButtons["LOOT_METHOD"].text = dropdownMenu.selectedLootMethod;
	L_UnitPopupButtons["LOOT_METHOD"].tooltipText = UnitLootMethod[GetLootMethod()].tooltipText;
	dropdownMenu.selectedLootThreshold = _G["ITEM_QUALITY"..GetLootThreshold().."_DESC"];
	L_UnitPopupButtons["LOOT_THRESHOLD"].text = dropdownMenu.selectedLootThreshold;
	...
	local showLootOptions = (IsInRaid() or (inInstance and instanceType == "raid")) and isLeader and not HasLFGRestrictions();
	local lootOption = showLootOptions and 1 or nil;

	L_UnitPopupButtons["LOOT_METHOD"].nested = lootOption;
	L_UnitPopupButtons["LOOT_THRESHOLD"].nested = lootOption;

	-- Set the selected opt out of loot option to the opt out of loot button text
	if ( GetOptOutOfLoot() ) then
		L_UnitPopupButtons["OPT_OUT_LOOT_TITLE"].text = format(OPT_OUT_LOOT_TITLE, L_UnitPopupButtons["OPT_OUT_LOOT_ENABLE"].text);
	else
		L_UnitPopupButtons["OPT_OUT_LOOT_TITLE"].text = format(OPT_OUT_LOOT_TITLE, L_UnitPopupButtons["OPT_OUT_LOOT_DISABLE"].text);
	end
			...
			if( L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] == 1 ) then
				...
				if ( previousWasSubsectionTitle ) then
					if ( not cntButton.isSubsectionTitle ) then
						L_UIDropDownMenu_AddSeparator(info, L_UIDROPDOWNMENU_MENU_LEVEL);
						L_UnitPopup_AddDropDownButton(info, dropdownMenu, L_UnitPopupButtons[subsectionTitleValue], subsectionTitleValue, L_UIDROPDOWNMENU_MENU_LEVEL);
					else
						L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][subsectionTitleIndex] = 0;
					end
				...
				elseif (strsub(value, 1, 15) == "RAID_DIFFICULTY" and (strlen(value) > 15) ) then
					if ( isDynamicInstance ) then
						...
						if ( IsLegacyDifficulty(instanceDifficultyID) ) then
							...
							if ((instanceDifficultyID == DIFFICULTY_RAID10_NORMAL or instanceDifficultyID == DIFFICULTY_RAID25_NORMAL) and L_UnitPopupButtons[value].difficultyID == DIFFICULTY_PRIMARYRAID_NORMAL) then
								info.checked = true;
							elseif ((instanceDifficultyID == DIFFICULTY_RAID10_HEROIC or instanceDifficultyID == DIFFICULTY_RAID25_HEROIC) and L_UnitPopupButtons[value].difficultyID == DIFFICULTY_PRIMARYRAID_HEROIC) then
								info.checked = true;
							end
					...
					if ( toggleDifficultyID and CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID) ) then
						info.disabled = nil;
					end
				elseif (strsub(value, 1, 22) == "LEGACY_RAID_DIFFICULTY" and (strlen(value) > 15) ) then
					...
					if ( toggleDifficultyID and not GetRaidDifficultyID() == DIFFICULTY_PRIMARYRAID_MYTHIC and CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID) ) then
						info.disabled = nil;
					end
	...
	for index, value in ipairs(L_UnitPopupMenus[which]) do
		if( L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] == 1 ) then
			...
			if ( previousWasSubsectionTitle ) then
				if ( not cntButton.isSubsectionTitle ) then
					L_UIDropDownMenu_AddSeparator(info, L_UIDROPDOWNMENU_MENU_LEVEL);
					L_UnitPopup_AddDropDownButton(info, dropdownMenu, L_UnitPopupButtons[subsectionTitleValue], subsectionTitleValue);
				else
					L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][subsectionTitleIndex] = 0;
				end
				subsectionTitleIndex = nil;
...
function L_UnitPopup_HideButtons ()
	...
	for index, value in ipairs(L_UnitPopupMenus[L_UIDROPDOWNMENU_MENU_VALUE] or L_UnitPopupMenus[dropdownMenu.which]) do
		L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] = 1;
		if ( value == "TRADE" ) then
			if ( not canCoop or not isPlayer ) then
				L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
			end
	   ...
	   elseif ( value == "MOVE_FOCUS_FRAME" ) then
		...
		elseif ( value == "DUNGEON_DIFFICULTY" ) then
			if ( UnitLevel("player") < 65 and GetDungeonDifficultyID() == L_UnitPopupButtons[value].defaultDifficultyID ) then
				L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
			end
		elseif ( value == "RAID_DIFFICULTY" ) then
			if ( UnitLevel("player") < MAX_PLAYER_LEVEL_TABLE[LE_EXPANSION_WRATH_OF_THE_LICH_KING] and GetRaidDifficultyID() == L_UnitPopupButtons[value].defaultDifficultyID ) then
				L_UnitPopupShown[L_UIDROPDOWNMENU_MENU_LEVEL][index] = 0;
			end
...
function L_UnitPopup_OnUpdate (elapsed)
	...
	for index, value in ipairs(L_UnitPopupFrames) do
		if ( L_UIDROPDOWNMENU_OPEN_MENU == _G[value] ) then
			break;
		elseif ( index == #L_UnitPopupFrames ) then
			return;
		end
	end
		...
		if ( dropdownFrame ) then
			...
			for index, value in ipairs(L_UnitPopupMenus[dropdownFrame.which]) do
				if ( L_UnitPopupShown[level][index] == 1 ) then
					...
					if ( L_UnitPopupButtons[value].isUninteractable or
						(L_UnitPopupButtons[value].dist > 0 and not CheckInteractDistance(dropdownFrame.unit, L_UnitPopupButtons[value].dist)) or (L_UnitPopupButtons[value].disabledInKioskMode and IsKioskModeEnabled())) then
						enable = false;
					end
					...
					elseif ( ( strsub(value, 1, 15) == "RAID_DIFFICULTY" ) and ( strlen(value) > 15 ) ) then
						...
						if (toggleDifficultyID) then
							enable = CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID);
						end
						if (L_UnitPopupButtons[value].difficultyID == DIFFICULTY_PRIMARYRAID_MYTHIC and UnitLevel("player") < MAX_PLAYER_LEVEL_TABLE[LE_EXPANSION_MISTS_OF_PANDARIA]) then
							enable = false;
						end
					elseif ( ( strsub(value, 1, 22) == "LEGACY_RAID_DIFFICULTY" ) and ( strlen(value) > 22 ) ) then
						...
						if (toggleDifficultyID) then
							if (IsLegacyDifficulty(toggleDifficultyID)) then
								notClickable = CheckToggleDifficulty(toggleDifficultyID, L_UnitPopupButtons[value].difficultyID);
							end
							enable = false;
						end
					elseif ( value == "CONVERT_TO_PARTY" ) then
					...
					elseif ( value == "CHARACTER_FRIEND" ) then
						if ( not UnitCanCooperate("player", L_UIDROPDOWNMENU_INIT_MENU.unit) ) then
							...
							if ( not UnitIsSameServer(L_UIDROPDOWNMENU_INIT_MENU.unit) or GetFriendInfo(UnitName(L_UIDROPDOWNMENU_INIT_MENU.unit)) ) then
								enable = false;
							end
						end
                    elseif ( value == "MASTER_LOOTER" ) then
...
function L_UnitPopup_OnClick (self)
	...
	elseif ( button == "FREE_FOR_ALL" ) then
		...
		L_UIDropDownMenu_SetButtonText(self:GetParent().parentLevel, self:GetParent().parentID, L_UnitPopupButtons[button].text);
